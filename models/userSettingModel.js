...
exports.CheckIfPasswordMatch = async (password, email) => {
  const [rows] = await db.query('SELECT COUNT(*) AS count FROM Users WHERE userPassword = ? AND userEmail = ?', [
    password,
    email,
  ]);
...
exports.UpdatePassword = async (password, email, currPassword) => {
  try {
    const [rows] = await db.execute('UPDATE Users SET userPassword = ? WHERE userEmail = ? AND userPassword = ?', [
      password,
      email,
      currPassword,
    ]);
