...
exports.updateTemplate = async function (id, newContent) {
	...
	const [rows] = await db.query(
		'UPDATE Templates SET templateContent = ?, templateLastUpdate = ? WHERE templateId = ?',
		[newContent, lastUpdate, id]
	);
...
exports.templateCreation = async function (title, content) {
	...
	const [rows] = await db.execute(
		'INSERT INTO Templates (templateTitle, templateContent, templateCreationDate, templateLastUpdate) VALUES (?,?,?,?)',
...
exports.replacePlaceholder = async function (id, domain) {
	...
	try {
		...
		); // det er vist kun SUBDOMAIN01 som skal være det som brugeren skriver. resten skal være random. skal lige dobbelt tjekkes
